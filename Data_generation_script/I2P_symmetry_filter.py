...
root_dir="C:/Users/ay162/Desktop/research/New_data/new_csg_png/new_csg_png"
destination="C:/Users/ay162/Desktop/research/New_data/I2P"
potent_data=7500
...
def get_sym(img1,img2,img3):
    ...
    img3=img3.T
    if j>np.size(np.nonzero(img2-img3)):
        j=np.size(np.nonzero(img2-img3))
    img1v=img1[:,::-1]
    img1v=img1v[::-1,:]
    img1v=img1-img1v
    if j>np.size(np.nonzero(img1v)):
        j=j>np.size(np.nonzero(img1v))
    return j
...
for dic in dics:
    ...
    if blank:
        ...
        j = get_sym(threshf, threshr, thresht)
    if j==0:
        counter+=1
    sym.append(j)
