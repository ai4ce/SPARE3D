...
def Generate_task(fname, answer_dic=answer,Path_output=pathwrite,args=args):
	...
	try:
		...
		for vp in viewpoints:
			converter.export_shape_to_svg(shape=shp, filename=MotherDic+model_number+"_"+vp+".svg", proj_ax=converter.DIRS[vp],scale=sc)
		
        #### generate correct answer 
		index=random.choice(index_list)
		converter.export_shape_to_svg(shape=shp, filename=MotherDic+answer_dic[model_number]+".svg", proj_ax=converter.DIRS[index],scale=sc)
		...
		for Vp in Index_list:
			converter.export_shape_to_svg(shape=shp, filename=MotherDic+answer_number[j]+".svg", proj_ax=converter.DIRS[Vp],scale=sc)
			j=j+1
		return 1
